...
2. **Should GPS/location metadata be stripped from public photos?**
   ...
   - See the [EXIF Metadata Stripping](#exif-metadata-stripping) section below for technical details if the board decides to strip metadata
...
- **Internal** (shared albums with staff accounts): Content for staff and trusted users
...
The school likely already has some form of photo clause or media release. If so:
...
4. The existing clause may already cover the archive's use cases -- confirm with the board
...
- **Enable face recognition** for admin and internal use -- it makes the archive far more useful
...
## Appendix: EXIF Metadata Stripping

This section covers the technical steps for stripping metadata from photos. **Only needed if the board decides metadata should be removed from publicly shared content.**
...
Install `exiftool` on the server:
...
### Immich's Built-in Metadata Toggle

Immich shared links include an option to **show or hide metadata**. Toggling metadata off in the shared link settings prevents viewers from seeing EXIF data in the UI, but the data still exists in the file if they download it. If downloads are allowed and metadata must be removed, use `exiftool` as described above.
...
Only strip GPS, serial numbers, and other personally identifying fields if the board requires it.
